...
// TODO: Make these functions to extension functions
public class ControllerUtil {
    ...
    public boolean cannotEditCircle(Long circleId, HttpServletRequest request) {
        ...
        user.getPermissions();
        return !(user.getPermissions().contains("ROLE_LEADER") && user.getPermissions().contains("CIRCLE_" + circleId) || user.getSysadmin());
    }
    
    public boolean cannotEditCircleNoPR(Long circleId, HttpServletRequest request) {
        UserEntity user = getUser(request);
        user.getPermissions();
        return !(user.getPermissions().contains("ROLE_LEADER") && user.getPermissions().contains("CIRCLE_" + circleId) && !user.getPermissions().contains("PR_" + circleId) || user.getSysadmin());
    }
